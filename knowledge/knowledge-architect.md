...
A fonte de verdade combinada vem de `scripts/mednotes/wiki_tree.py --max-depth 4 --audit`, que retorna a taxonomia canonica, a arvore real existente e uma auditoria dry-run; para leitura humana, use `scripts/mednotes/wiki_tree.py --max-depth 4 --audit --format text`. Conceitualmente, a taxonomia canonica tem 5 grandes areas: `1. Clínica Médica`, `2. Cirurgia`, `3. Ginecologia e Obstetrícia`, `4. Pediatria`, `5. Medicina Preventiva`. Essas grandes areas e as categorias canonicas sob elas sao fixas: nao invente sexta area, nova categoria canonica, grafia alternativa, singular/plural alternativo ou pasta intermediaria. Operacionalmente, a arvore real mostra quais pastas ja existem; reutilize os nomes exatamente como aparecem ali, incluindo acentos, underscores e plural. Se uma area ou categoria canonica estiver ausente como pasta fisica, a CLI pode materializar esse prefixo canonico. Qualquer nova pasta de taxonomia fora desse prefixo so pode ser criada quando for **uma unica folha nova** sob pai coerente; o dry-run exibira isso em `taxonomy_new_dirs`.

A hierarquia nao exige obrigatoriamente quatro niveis. O minimo operacional e `Grande Área/Categoria/Título.md`; grupos tematicos abaixo da categoria sao opcionais quando ja existem ou quando o dry-run autoriza uma unica folha nova. Assim, `1. Clínica Médica/Semiologia` + titulo `Aferição de Temperatura` gera `1. Clínica Médica/Semiologia/Aferição de Temperatura.md`; nao crie `1. Clínica Médica/Clínica Médica/Semiologia`.
...
Distribuicao canonica: `1. Clínica Médica` inclui Cardiologia, Dermatologia, Endocrinologia, Gastroenterologia, Geriatria, Hematologia, Imunologia, Infectologia, Medicina Interna, Nefrologia, Neurologia, Nutrologia, Oncologia, Pneumologia, Reumatologia, Semiologia e Psiquiatria; `2. Cirurgia` inclui Cirurgia Geral, Clínica Cirúrgica, Oftalmologia, Urologia, Trauma e Anestesiologia; `3. Ginecologia e Obstetrícia` consolida Ginecologia/Obstetrícia; `4. Pediatria` inclui Pediatria, Neonatologia, Puericultura e Infecto Pediátrica; `5. Medicina Preventiva` inclui Medicina Preventiva, SUS, Epidemiologia, Ética Médica e Saúde do Trabalho.
